...
class UserdataController extends Controller
{
    ...
    public function actionCreate()
    {
		...
		if ($userdata->load(Yii::$app->request->post())) {
			$user->username = $_POST['Userdata']['username'];
			$user->auth_key = Yii::$app->security->generateRandomString();
			$user->password_hash = Yii::$app->security->generatePasswordHash($_POST['Userdata']['password']);
			$user->email = $_POST['Userdata']['email'];
			$user->created_at = strtotime('now');
			$user->updated_at = strtotime('now');
			if($user->save()){
				$userdata->user_id = $user->id;
				$userdata->fullname = $_POST['Userdata']['fullname'];
				$userdata->save();
				return $this->redirect(['view', 'id' => $userdata->ui_id]);
			}
        } else {
    ...
    public function actionUpdate($id)
    {
		...
		if ($model->load(Yii::$app->request->post())) {
			$model->fullname = $_POST['Userdata']['fullname'];
    ...
    public function actionDelete($id)
    {
        ...
        $user = User::findOne($userdata->user_id);
        $user->delete();
