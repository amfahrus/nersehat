...
function search(target) {
    ...
    var groups = {
        unit: [$('<optgroup label="Keluarga">'), false],
        role: [$('<optgroup label="Roles">'), false],
        permission: [$('<optgroup label="Permission">'), false],
    };
    $.each(_opts.items[target], function (group, vals) {
		$.each(vals, function (id, name) {
			//console.log(vals);
			if (id.indexOf(q) >= 0) {
				$('<option>').text(name).val(id).appendTo(groups[group][0]);
				groups[group][1] = true;
			}
		});
    });
